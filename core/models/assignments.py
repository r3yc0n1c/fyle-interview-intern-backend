...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(assignment.state is not AssignmentStateEnum.DRAFT,"assignment in draft state can not be graded")
        
        if auth_principal.teacher_id:
            assertions.assert_valid(
                assignment.teacher_id == auth_principal.teacher_id, 
                f"assignment {_id} was submitted to teacher {assignment.teacher_id} and not teacher {auth_principal.teacher_id}"
            )
    ...
    def get_assignments_by_student(cls, student_id):
        assignments = cls.filter(cls.student_id == student_id).all()
